...
# Copyright (c) 2017-2024 The txbtc Core developers
...
from test_framework.test_framework import txbtcTestFramework
from test_framework.util import create_confirmed_utxos, hex_str_to_bytes, assert_equal


class ChainstateWriteCrashTest(txbtcTestFramework):
    ...
    def restart_node(self, node_index, expected_tip):
        ...
        while time.time() - time_start < 120:
            ...
            try:
                ...
                # If txbtcd exits, then try again.  wait_for_node_exit()
                # should raise an exception if txbtcd doesn't exit.
                self.log.debug("Wait for node exit ~~ during restart, node: %s", node_index)
                self.wait_for_node_exit(node_index, timeout=10)
            self.crashed_on_restart += 1
            time.sleep(1)

        # If we got here, txbtcd isn't coming back up on restart.  Could be a
        # bug in txbtcd, or we've gotten unlucky with our dbcrash ratio --
