...
# Copyright (c) 2017-2021 The txbtc Core developers
...
`test/functional/test_framework/test_framework.py:txbtcTestFramework.main`.
...
def main():
    ...
    tmpdir = "%s/txbtc_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    # Check that the build was configured with wallet, utils, and txbtcd
    enable_wallet = config["components"].getboolean("ENABLE_WALLET")
    enable_cli = config["components"].getboolean("ENABLE_UTILS")
    enable_txbtcd = config["components"].getboolean("ENABLE_txbtcD")
    if not (enable_wallet and enable_cli and enable_txbtcd):
        print("No functional tests to run. Wallet, utils, and txbtcd must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, use_term_control, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, last_loop=False):
    # Warn if txbtcd is already running (unix only)
    if args is None:
        args = []
    try:
        if subprocess.check_output(["pidof", "txbtcd"]) is not None:
            print("%sWARNING!%s There is already a txbtcd process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "txbtcD" not in os.environ:
        os.environ["txbtcD"] = build_dir + '/src/txbtcd' + exeext
        os.environ["txbtcCLI"] = build_dir + '/src/txbtc-cli' + exeext
...
class RPCCoverage:
    ...
    commands per `txbtc-cli help` (`rpc_interface.txt`).
